...
#define TARGET_ADDR_COUNT 100

static const char *device_path = "/dev/bone/i2c/2";
static const uint8_t slave_addr_offset = 16;
static const uint8_t slave_addr_count = 100;
static const uint8_t polling_interval_sec = 1;

static int device_fd = 0;

bool target_addr_list[TARGET_ADDR_COUNT] = {0};
bool target_addr_set[TARGET_ADDR_COUNT] = {false};
...
static void scan_i2c_bus(void)
{
    ...
    explicit_bzero(target_addr_set, TARGET_ADDR_COUNT);
    explicit_bzero(target_addr_list, TARGET_ADDR_COUNT);
    ...
    for (uint8_t i = 0; i < TARGET_ADDR_COUNT; i++)
    {
        ...
        if (read_bytes > 0)
        {
            sprintf(syslog_buff, "Detected target device at address [0x%X].", addr);
            syslog_append(syslog_buff);
            target_addr_set[i] = true;
            target_addr_list[list_index] = addr;
            list_index++;
        }
    }

    sprintf(syslog_buff, "Concluded I2C bus scan, %d devices detected.", list_index);
    syslog_append(syslog_buff);
}
